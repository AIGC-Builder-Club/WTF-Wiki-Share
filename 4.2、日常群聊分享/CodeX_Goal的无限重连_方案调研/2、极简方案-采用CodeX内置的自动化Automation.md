		...
		- 模型    GPT-5.6-Sol
			- （~~后来改为  Luna  Max~~）
				- （这儿比较容易混淆，都不是最终测试的答案）
					- 此处，有一个非常隐蔽的严重Bug：
						- 【你在这里设置的模型】，会被带到【主力任务】。
							- 也就是说，会把【主力任务】从【Sol Max】覆盖为【Luna Max】
								- （这个，我多次检查过）
						- 所以，应该————【定时任务对话】手动设置为【Luna Max】，而【此处设置  改为  Sol  Max】
					- 但是后来又发现，可能还是应该在【`派发精确命令 /goal resume（model=gpt-5.6-luna、thinking=max）`】  这一句上来改？
			- （先保留为  Sol  Max  吧）
		- 推理（强度）    Max
	- 频率
		- 重复    每天、每小时、每周
...
- 最后生成的产物，示例【`  .codex/automations/goal-019f8d10/automation.toml  `】：    （时间，可以改一下；【5分钟】还是太密集了。改为20分钟吧    ）
	- 另外，还产生了一个需要引用的【mjs脚本】——通过桌面端同版本  ` Codex App Server 执行：thread/goal/get → thread/goal/set → thread/goal/get `  
		- [/Users/ycw/.codex/automations/goal-019f8d10/thread_goal_rpc.mjs  已备份到本GitHub目录下](Root/_外部Publish/4.2、日常群聊分享/CodeX_Goal的无限重连_方案调研/2、极简方案-采用CodeX内置的自动化Automation/thread_goal_rpc.mjs)
	...
	- v3版本
		...
		- 然后发现，————无法【监控多个任务】、
			- 无法【监控所有进行中的Goal任务（我觉得，这个还是有必要的。）】、
				- 已完成：    [对现有Goal的所有阻塞状态分析](Root/_外部Publish/4.2、日常群聊分享/CodeX_Goal的无限重连_方案调研/2、极简方案-采用CodeX内置的自动化Automation/对现有Goal的所有阻塞状态分析.md)
	- v2版本
		- …………  运行似乎还算正常，再观察一阵子。
		- 1
